...
void PoseGraph::optimize(bool exclude_registration_constraints) {
  ...
  problem_options_.local_parameterization_ownership = ceres::Ownership::DO_NOT_TAKE_OWNERSHIP;
  problem_ptr_.reset(new ceres::Problem(problem_options_));

  //add registration constraint
  constraints_collection_.addConstraintsToProblem(node_collection_, problem_ptr_.get(), exclude_registration_constraints);
